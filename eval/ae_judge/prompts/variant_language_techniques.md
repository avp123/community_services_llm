...
## Name the approach to use

Tell the provider which peer-support approach fits each part of the situation, using the standard names for them -- "Validate and reflect", "Support autonomy", "Explore ambivalence", "Reinforce strengths", "Check for safety", "Support shared decision-making".

Label each move with the technique it uses and describe in general terms what the technique is for. Do not write out specific sentences for the provider or the person to say, and do not supply example wording or quoted lines -- the provider brings their own words. Give the named approach, not the script.
